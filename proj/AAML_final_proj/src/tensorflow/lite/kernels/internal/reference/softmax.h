...
inline void Softmax(const SoftmaxParams& params,
  ...
  const int trailing_dim = input_shape.DimensionsCount() - 1;
  const int outer_size =
      MatchingFlatSizeSkipDim(input_shape, trailing_dim, output_shape);// is 1
  const int depth =
      MatchingDim(input_shape, trailing_dim, output_shape, trailing_dim);// is 10
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    for (int c = 0; c < depth; ++c) {
      //max_in_row = std::max(max_in_row, input_data[i * 10 + c]);
      max_in_row = std::max(max_in_row, input_data[c]);
    }
    ...
    for (int c = 0; c < depth; ++c) {
      int32_t input_diff =
          static_cast<int32_t>(input_data[c]) - max_in_row;
    ...
    for (int c = 0; c < depth; ++c) {
      ...
      if (input_diff >= diff_min) {
        ...
        output_data[c] = static_cast<OutputT>(std::max(
            std::min(shifted_output,
                     static_cast<int32_t>(std::numeric_limits<OutputT>::max())),
            static_cast<int32_t>(std::numeric_limits<OutputT>::min())));
      } else {
        output_data[c] = std::numeric_limits<OutputT>::min();
      }
    }
  }
  //printf("Doing softmax!\n");
}
