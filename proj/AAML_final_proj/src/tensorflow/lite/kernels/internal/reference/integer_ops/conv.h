...
inline void ConvPerChannel(
  ...
  //cfu_op0(2, input_offset & 0x1FF, 0);//set input offset
  cfu_op0(2, 128, 0);
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              if(input_depth == 3){
                ...
                for(int d=0;d<3;d++){
                  ...
                  //input_val = (input_val << 8) + (int32_t(input_ptr[normal_depth]) & 0xFF);
                  //filter_val = (filter_val << 8) + (int32_t(filter_ptr[normal_depth]) & 0xFF);
                  acc = cfu_op0(0, input_val, filter_val);
              }

              for (int in_channel = 0; in_channel < normal_depth;
                   in_channel+=16) { 
                ...
                input_val = *reinterpret_cast<const int32_t*>(input_ptr + in_channel + 4);
			          filter_val = *reinterpret_cast<const int32_t*>(filter_ptr + in_channel + 4);
                cfu_op0(0, input_val, filter_val);
                input_val = *reinterpret_cast<const int32_t*>(input_ptr + in_channel + 8);
			          filter_val = *reinterpret_cast<const int32_t*>(filter_ptr + in_channel + 8);
                cfu_op0(0, input_val, filter_val);
                input_val = *reinterpret_cast<const int32_t*>(input_ptr + in_channel + 12);
			          filter_val = *reinterpret_cast<const int32_t*>(filter_ptr + in_channel + 12);
			          acc = cfu_op0(0, input_val, filter_val);
              }
            }
          }
