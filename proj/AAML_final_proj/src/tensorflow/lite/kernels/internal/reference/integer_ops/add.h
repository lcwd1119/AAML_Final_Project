...
inline int8_t AddFunc(int8_t x, int8_t y, const ArithmeticParams& params) {
  ...
  const int32_t shifted_input1_val = input1_val * (1 << params.left_shift);//origin: 1 << params.left_shift
  const int32_t shifted_input2_val = input2_val * (1 << params.left_shift);//origin: 1 << params.left_shift
  
  const int32_t scaled_input1_val =
      MultiplyByQuantizedMultiplierSmallerThanOneExp(
          shifted_input1_val, params.input1_multiplier, params.input1_shift);//params.input1_shift -> -2
  
  
  const int32_t scaled_input2_val =
      MultiplyByQuantizedMultiplierSmallerThanOneExp(
          shifted_input2_val, params.input2_multiplier, params.input2_shift);//params.input2_multiplier, params.input2_shift -> 1073741824, 0
  
  const int32_t raw_sum = scaled_input1_val + scaled_input2_val;
  
  const int32_t raw_output =
      MultiplyByQuantizedMultiplierSmallerThanOneExp(
          raw_sum, params.output_multiplier, params.output_shift) + params.output_offset;// + params.output_offset -> -128
         
  const int32_t clamped_output =
      std::min(params.quantized_activation_max,
               std::max(params.quantized_activation_min, raw_output));
  //printf("min = %ld\n",(int32_t)params.quantized_activation_min);
  //printf("max = %ld\n",(int32_t)params.quantized_activation_max);
